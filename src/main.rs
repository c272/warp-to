...
* [1] Term-based search.
 ...
 `warp-to` will search both higher and lower heirarchies for the first term, and lower heirarchies only for following terms.

 For example, the below will search for a directory named "foo", and then try to find a child directory somewhere named "bar".
   ~> w foo bar
 You can also chain as many terms together as you like.
   ~> w foo bar fugu hoge

 If you know a concrete directory structure that you want to search for, you can do so using slashes as delimiters.
 For example, the below would search for a concrete directory structure "foo/bar/fugu", followed by a child directory
 somewhere named "hoge".
   ~> w foo/bar/fugu hoge

 The special path characters you are used to on Unix-like systems also work with `warp-to`, such as root ("/") and home ("~").
   ~> w / hoge
   ~> w ~ doom

 In addition to system directories, you can also search for child directories of user-configured shortcuts, seen in [3].
 The below would first resolve the shortcut "foobar", and then search for some child directory named "doom".
   ~> w +foobar doom
...
* [3] Shortcut-based navigation.
 ...
 `$CONFIG/warp-to` on Unix-like systems, or `%APPDATA%/warp-to` on Windows.
   ~> w +foobar

Options:
  -d, --distance <DIST>  The maximum distance of the search, default 5.
                         Increasing this value will extend the search time of the program, but provide better results.
